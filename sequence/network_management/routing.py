...
def add_forwarding_rule(self, dst: str, next_node_arr: list, prob_arr: list):
    ...
    """
    assert abs(sum(prob_arr) - 1.0) < 1e-6 #sum of probabilities must be equal to 1
    assert len(next_node_arr) == len(prob_arr) #size of the next node array must equal to the size of the corresponding probability array
